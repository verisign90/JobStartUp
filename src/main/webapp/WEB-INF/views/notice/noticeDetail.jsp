      ...
      <section class="view">
        ...
        <section class="posting">
          ...
          <c:if test="${!empty noticeDTO.not_modDate}">
           <h5><fmt:parseDate value="${noticeDTO.not_modDate}" pattern="yyyy-MM-dd'T'HH:mm" var="parsedDateTime" type="both" />
               <fmt:formatDate value="${parsedDateTime}" pattern="YYYY-MM-dd HH:mm:ss"/></h5>
          </c:if>
           <h5><fmt:parseDate value="${noticeDTO.not_regDate}" pattern="yyyy-MM-dd'T'HH:mm" var="parsedDateTime" type="both" />
               <fmt:formatDate value="${parsedDateTime}" pattern="YYYY-MM-dd HH:mm:ss"/></h5>
          </div>
          ...
          <div class="file">
            <c:if test="${!empty noticeDTO.fileDTOList}">
              <c:forEach var="file" items="${noticeDTO.fileDTOList}">
                <span id="img"><img src="${cPath}/image/notice/${file.notFile_savName}" style="width:300px;"/></span>
              </c:forEach>
            </c:if>
          </div>
        </section>
