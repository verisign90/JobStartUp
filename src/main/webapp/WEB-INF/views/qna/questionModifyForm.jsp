...
<body>
  <article class="qna">
    ...
    <section class="contents">
      <h4>문의하기</h4>
      <div class="inner">
      <form action="/qna/modify" enctype ="multipart/form-data" method="post" name="writeFrom" id="writeFrom" class="txt">
       <input type="hidden" value="${questionDTO.q_no}" id="q_no" name="q_no"/>
       <div class="wrap_input">
        ...
        <select class="select" name="q_category" id="q_category">
          ...
          <textarea name="q_content" id="textarea" rows="10" cols="30">${questionDTO.q_content}</textarea>
  ...
  <script>
    showUploadResult = function showUploadResult(){
        ...
        /* 기존에 저장된 이미지 div 반환 */
        makePreDiv = function(item, hiddenInput){
          var div = document.createElement('div');
          div.setAttribute('style', div_style);
          ...
          var fileSpanText = document.createTextNode(item.qfile_orgName);
          fileSpan.appendChild(fileSpanText);
          var btn = document.createElement('button');
          btn.setAttribute('type', 'button');
          btn.setAttribute('class', 'no-hover');
          btn.setAttribute('delFile', item.name);
          btn.setAttribute('style', btn_style);
          btn.onclick = function(ev){
            var ele = ev.srcElement;
            var p = ele.parentNode;
      ...
      fileListArray.forEach(function(item){
        ...
        hiddenInput.setAttribute('value', item.qfile_no);
    ...
    $(document).ready(function() {
       ...
       // radio checked & option selected 값 변경
       var checkedValue = ${typeJson};
    ...
    function selectOptionByValue(selectedValue) {
      var selectElement = document.getElementById('q_category');
